...
class GitHubMetricsDashboard:
    ...
    def create_contributor_summary(self, data):
        ...
        try:
            ...
            # Add comment and conversation metrics
            ...
            if all(col in contributor_summary.columns for col in comment_metrics_columns):
                ...
                with col1:
                    total_reviewer = contributor_summary['Total Reviewer Comments'].sum()
                    total_approver = contributor_summary['Total Approver Comments'].sum()
                    total_comments = total_reviewer + total_approver
                    st.metric(
                        label="Total Comments", 
                        value=total_comments
                ...
                with col2:
                    total_resolved = contributor_summary['Total Resolved Conversations'].sum()
                    total_unresolved = contributor_summary['Total Unresolved Conversations'].sum()
                    total_conversations = total_resolved + total_unresolved
                    resolution_rate = round(total_resolved / total_conversations * 100, 1) if total_conversations > 0 else 0
                    st.metric(
                        label="Conversation Resolution Rate", 
                        value=f"{resolution_rate}%"
                    )
                
                with col3:
                    if 'Breaking Change PRs' in contributor_summary.columns:
                        total_breaking = contributor_summary['Breaking Change PRs'].sum()
                        st.metric(
                            label="Breaking Change PRs", 
                            value=total_breaking
                        )
                ...
                # Add Contributors by Comment Activity table
                st.subheader("Contributors by Comment Activity")
                comment_activity = contributor_summary[['Contributor', 'Total Reviewer Comments', 'Total Approver Comments']].copy()
                
                # Add total comments column
                comment_activity['Total Comments'] = comment_activity['Total Reviewer Comments'] + comment_activity['Total Approver Comments']
                
                # Sort by total comments
                comment_activity = comment_activity.sort_values(by='Total Comments', ascending=False).head(10)
                
                # Display the table
                st.dataframe(comment_activity, use_container_width=True)
            ...
            if 'Health Percentage' in contributor_summary.columns:
                ...
                display_columns = [col if col != 'Unhealthy PRs' else 'Needs Attention PRs' for col in display_columns]
                if 'Unhealthy PRs' in healthy_contributors.columns:
                    healthy_contributors = healthy_contributors.rename(columns={'Unhealthy PRs': 'Needs Attention PRs'})
                
                # Add breaking change count if available
                if 'Breaking Change PRs' in healthy_contributors.columns:
                    display_columns.append('Breaking Change PRs')
                
                # Add comment metrics if available
                for col in ['Total Reviewer Comments', 'Total Approver Comments']:
                    if col in healthy_contributors.columns:
                        display_columns.append(col)
                ...
                # Make sure all display columns exist in the dataframe
                display_columns = [col for col in display_columns if col in healthy_contributors.columns or (col == 'Needs Attention PRs' and 'Unhealthy PRs' in healthy_contributors.columns)]
                
                if not display_columns:
                    st.warning("No contributor health data columns available for display")
                else:
                    st.dataframe(
            ...
            if 'repository' in contributor_detail.columns and 'passed_checks' in contributor_detail.columns and 'failed_checks' in contributor_detail.columns:
                ...
                check_by_repo = contributor_detail.groupby('repository').agg({
                    'passed_checks': 'sum',
                    'failed_checks': 'sum',
                    'total_commits': 'sum'
                }).reset_index()
