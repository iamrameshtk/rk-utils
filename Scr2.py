...
class CloudDataFusionClient:
    ...
    def _make_request(self, method: str, url: str, test_case: str, description: str, operation_type: str = "Instance Level Operation", **kwargs) -> Tuple[requests.Response, TestResult]:
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not         with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
...
class ReportGenerator:
    ...
    def display_results_table(test_results: List[TestResult]):
        ...
        for result in test_results:
            table_data.append([
                result.test_case[:35] + '...' if len(result.test_case) > 35 else result.test_case,
                result.method,
                result.status,
                result.response_code,
                f"{result.execution_time:.3f}s",
                result.description[:55] + '...' if len(result.description) > 55 else result.description
            ])
        ...
        # Category breakdown
        namespace_tests = len([r for r in test_results if 'NAMESPACE' in r.test_case])
        system_tests = len([r for r in test_results if 'SYSTEM' in r.test_case])
        
        print(f"\n📋 TEST CATEGORIES:")
        print(f"   Namespace Operations: {namespace_tests}")
        print(f"   System Admin Operations: {system_tests}")
        print(f"   Other Operations: {total_tests - namespace_tests - system_tests}")
