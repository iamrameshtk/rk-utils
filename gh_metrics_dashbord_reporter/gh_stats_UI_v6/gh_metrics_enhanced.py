...
class GitHubMetricsReporter:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                rate_response = requests.get(
                    f"{self.base_url}/rate_limit", headers=headers
                )

                if rate_response.status_code == 200:
                    limits = rate_response.json()["rate"]
                    self.logger.info(
                        f"API Rate Limits: {limits['remaining']}/{limits['limit']} remaining"
                    )
    ...
    def get_pr_details(self, headers, repo, pr_number):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/repos/{repo}/pulls/{pr_number}", headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to fetch PR details for {repo}#{pr_number}: {response.status_code}"
                )
                return {}

        except Exception as e:
            self.logger.error(
                f"Error fetching PR details for {repo}#{pr_number}: {str(e)}"
            )
            return {}
    ...
    def get_user_details(self, headers, username):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/users/{username}", headers=headers
            )
    ...
    def get_org_membership(self, headers, org, username):
        ...
        try:
            membership_response = requests.get(
                f"{self.base_url}/orgs/{org}/memberships/{username}", headers=headers
            )

            if membership_response.status_code == 200:
                # Get teams
                teams_response = requests.get(
                    f"{self.base_url}/orgs/{org}/teams", headers=headers
                )
    ...
    def get_check_runs(self, headers, repo, commit_sha):
        ...
        try:
            ...
            passed_checks = sum(
                1 for check in checks if check.get("conclusion") == "success"
            )
            failed_checks = sum(
                1
                for check in checks
    ...
    def get_pr_files(self, headers, repo, pr_number):
        ...
        try:
            ...
            while True:
                ...
                if response.status_code != 200:
                    self.logger.error(
                        f"Failed to fetch PR files for {repo}#{pr_number}: {response.status_code}"
                    )
                    break
            ...
            self.logger.error(
                f"Error fetching PR files for {repo}#{pr_number}: {str(e)}"
            )
            return {"file_list": [], "file_count": 0, "additions": 0, "deletions": 0}
    ...
    def is_feat_or_fix_pr(self, pr_title):
        ...
        if (
            title_lower.startswith("feat ")
            or title_lower.startswith("fix ")
            or title_lower.startswith("feat! ")
        ):
            return (True, is_breaking_change)
    ...
    def check_folder_in_files(self, file_list, folder_name):
        ...
        for file_path in file_list:
            if (
                file_path.startswith(f"{folder_name}/")
                or f"/{folder_name}/" in file_path
            ):
                return True
                    ...
                    try:
                        ...
                        if start_date <= created_at <= end_date:
                            ...
                            pr_details = self.get_pr_details(
                                headers, repo, pr["number"]
                            )
                            target_branch = (
                                pr_details.get("base", {}).get("ref", "")
                                if pr_details
                                else ""
                            )
                            ...
                            metrics["stats"]["total_additions"] += file_data[
                                "additions"
                            ]
                            metrics["stats"]["total_deletions"] += file_data[
                                "deletions"
                            ]
                            ...
                            if pr["state"] == "closed" and pr["closed_at"]:
                                ...
                                pr_duration_days = (
                                    datetime.now(self.utc) - created_at
                                ).days
                            ...
                            if pr_duration_days > self.pr_threshold_days:
                                ...
                                health_reasons.append(
                                    f"PR open > {self.pr_threshold_days} days"
                                )
                            ...
                            if label_count > self.max_labels_threshold:
                                ...
                                health_reasons.append(
                                    f"PR has > {self.max_labels_threshold} labels"
                                )
                            ...
                            metrics["stats"]["total_rc_versions"] += version_analysis[
                                "rc_versions"
                            ]
                            metrics["stats"]["total_npd_versions"] += version_analysis[
                                "npd_versions"
                            ]
                            metrics["stats"][
                                "total_stable_versions"
                            ] += version_analysis["stable_versions"]
                            ...
                            for review in reviews:
                                ...
                                if review_state == "APPROVED":
                                    ...
                                    if (
                                        review.get("body")
                                        and review.get("body").strip()
                                    ):
                                        approver_comments.append(
                            ...
                            for comment in review_comments:
                                thread_id = comment.get(
                                    "in_reply_to_id", comment.get("id")
                                )
                                ...
                                conversation_threads[thread_id]["commenters"].add(
                            ...
                            commits_response = requests.get(
                                pr["commits_url"], headers=headers
                            )
                            commits = (
                            ...
                            is_feat_fix, is_breaking_change = self.is_feat_or_fix_pr(
                                pr["title"]
                            )

                            # Check for examples, tests, and integration_tests folders in the changed files
                            has_examples = (
                                self.check_folder_in_files(
                                    file_data["file_list"], "examples"
                                )
                                if is_feat_fix
                                else False
                            )
                            ...
                            has_integration_tests = (
                                self.check_folder_in_files(
                                    file_data["file_list"], "integration_tests"
                                )
                                if is_feat_fix
                                else False
                            )
                        ...
                        self.logger.error(
                            f"Error processing PR #{pr.get('number', 'unknown')}: {str(e)}"
                        )
    ...
    def generate_pr_activity_report(
        self, all_metrics, output_dir, all_repositories=None
    ):
        """
        ...
        try:
            ...
            for repo, metrics in all_metrics.items():
                repo_summary = {
                    ...
                    "Open PRs": metrics["stats"]["total_prs"]
                    - metrics["stats"]["merged_prs"],
                    "Healthy PRs": metrics["stats"]["healthy_prs"],
                    "Unhealthy PRs": metrics["stats"]["unhealthy_prs"],
                    "Unhealthy Due to Duration": metrics["stats"][
                        "unhealthy_due_to_duration"
                    ],
                    "Unhealthy Due to Labels": metrics["stats"][
                        "unhealthy_due_to_labels"
                    ],
                ...
                if metrics["stats"]["total_prs"] > 0:
                    repo_summary["Health Percentage"] = round(
                        (
                            metrics["stats"]["healthy_prs"]
                            / metrics["stats"]["total_prs"]
                        )
                        * 100,
                        1,
                    )
                ...
                for pr in metrics["pull_requests"]:
                    ...
                    record = {
                        ...
                        "Health Reasons": (
                            ", ".join(pr["health_reasons"])
                            if pr["health_reasons"]
                            else "N/A"
                        ),
                        ...
                        "Check Success Rate": (
                            round(
                                (
                                    pr["passed_checks"]
                                    / (pr["passed_checks"] + pr["failed_checks"])
                                )
                                * 100,
                                1,
                            )
                            if (pr["passed_checks"] + pr["failed_checks"]) > 0
                            else "N/A"
                        ),
                        "Changed Files": ", ".join(pr["file_list"][:5])
                        + ("..." if len(pr["file_list"]) > 5 else ""),
                        "Is Feature/Fix PR": (
                            "Yes" if pr.get("is_feat_fix_pr", False) else "No"
                        ),
                        "Is Breaking Change": (
                            "Yes" if pr.get("is_breaking_change", False) else "No"
                        ),
                        "Has Examples": (
                            "Yes"
                            if pr.get("has_examples", False)
                            else "No" if pr.get("is_feat_fix_pr", False) else "N/A"
                        ),
                        "Has Tests": (
                            "Yes"
                            if pr.get("has_tests", False)
                ...
                if metrics["stats"]["total_prs"] > 0:
                    repo_summary["Avg PR Duration (days)"] = round(
                        total_duration / metrics["stats"]["total_prs"], 1
                    )
            ...
            if all_repositories:
                ...
                processed_repos = [
                    summary.get("Repository") for summary in summary_data
                ]
            ...
            pr_df["PR Health"] = pr_df["PR Health"].apply(
                lambda x: f"❌ {x}" if x == "Needs Attention" else f"✅ {x}"
            )
            ...
            with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
                ...
                self._format_excel_sheet(
                    writer.sheets["PR Activity"], pr_df, writer.book
                )

                # Write Summary data
                summary_df.to_excel(
                    writer, sheet_name="Repository Summary", index=False
                )
                self._format_summary_sheet(
                    writer.sheets["Repository Summary"], summary_df, writer.book
                )
    ...
    # In gh_metrics_enhanced.py, modify the generate_contributor_report method
    # Following code to track contributors from all commits, not just from PRs in the date range
    ...
    def fetch_additional_contributor_commits(self, headers, repo, start_date, end_date):
        ...
        try:
            self.logger.debug(
                f"Fetching direct commits for {repo} between {start_date.date()} and {end_date.date()}"
            )
            ...
            while True:
                ...
                if response.status_code != 200:
                    self.logger.error(
                        f"Failed to fetch commits for {repo}: {response.status_code}"
                    )
                    break
                ...
                for commit in commits:
                    try:
                        ...
                        self.logger.error(
                            f"Error processing commit in {repo}: {str(e)}"
                        )
                    continue
            ...
            self.logger.info(
                f"Found {len(contributor_commits)} contributors who made commits in {repo} between {start_date.date()} and {end_date.date()}"
            )
        ...
        try:
            ...
            for repo, metrics in all_metrics.items():
                ...
                for pr in metrics["pull_requests"]:
                    ...
                    stats["total_reviewer_comments"] += pr.get(
                        "total_reviewer_comments", 0
                    )
                    stats["total_approver_comments"] += pr.get(
                        "total_approver_comments", 0
                    )
                    stats["total_resolved_conversations"] += pr.get(
                        "total_resolved_conversations", 0
                    )
                    stats["total_unresolved_conversations"] += pr.get(
                        "total_unresolved_conversations", 0
                    )
                    ...
                    for commit in pr["commits"]:
                        if commit.get("date"):
                            commit_date = datetime.strptime(
                                commit["date"], "%Y-%m-%dT%H:%M:%SZ"
                            )
                            commit_date = self.utc.localize(commit_date)

                            if (
                                author not in contributor_first_date
                                or commit_date < contributor_first_date[author]
                            ):
                                contributor_first_date[author] = commit_date

                            if (
                                author not in contributor_last_date
                                or commit_date > contributor_last_date[author]
                            ):
                ...
                for author, stats in contributors.items():
                    if (
                        author in contributor_first_date
                        and author in contributor_last_date
                    ):
                        ...
                        stats["avg_commits_per_day"] = round(
                            stats["total_commits"] / active_days, 2
                        )
                    ...
                    all_contributors_data[author]["total_commits"] += stats[
                        "total_commits"
                    ]
                    all_contributors_data[author]["total_prs"] += stats["total_prs"]
                    all_contributors_data[author]["healthy_prs"] += stats["healthy_prs"]
                    all_contributors_data[author]["unhealthy_prs"] += stats[
                        "unhealthy_prs"
                    ]
                    all_contributors_data[author]["passed_checks"] += stats[
                        "passed_checks"
                    ]
                    all_contributors_data[author]["failed_checks"] += stats[
                        "failed_checks"
                    ]
                    all_contributors_data[author]["rc_versions"] += stats["rc_versions"]
                    all_contributors_data[author]["npd_versions"] += stats[
                        "npd_versions"
                    ]
                    all_contributors_data[author]["stable_versions"] += stats[
                        "stable_versions"
                    ]
                    all_contributors_data[author]["total_reviewer_comments"] += stats[
                        "total_reviewer_comments"
                    ]
                    all_contributors_data[author]["total_approver_comments"] += stats[
                        "total_approver_comments"
                    ]
                    all_contributors_data[author][
                        "total_resolved_conversations"
                    ] += stats["total_resolved_conversations"]
                    all_contributors_data[author][
                        "total_unresolved_conversations"
                    ] += stats["total_unresolved_conversations"]
                    all_contributors_data[author]["breaking_change_prs"] += stats[
                        "breaking_change_prs"
                    ]

            # Modified the main generate_contributor_report method to use the new function
            # This code block after all the PR processing but before adding contributors with no activity

            # Fetch additional contributors who made commits in PRs outside the date range
            # In the fetch_additional_contributor_commits code block, check that parameters exist before using them:
            if headers and start_date and end_date:
                for repo in all_metrics:
                    ...
                    repo_contributors = self.fetch_additional_contributor_commits(
                        headers, full_repo, start_date, end_date
                    )
                ...
                if all_contributors:
                    ...
                    processed_contributors = [
                        data.get("contributor") for data in contributor_data
                    ]
                ...
                for contributor in all_contributors:
                    if contributor not in processed_contributors:
                        self.logger.info(
                            f"Adding contributor with no activity: {contributor}"
                        )
                        contributor_data.append(
                            {
                                "repository": "N/A",
                ...
                summary_data.append(
                    {
                        ...
                        "Repository List": ", ".join(list(stats["repositories"])[:3])
                        + ("..." if len(stats["repositories"]) > 3 else ""),
                        ...
                        "Health Percentage": (
                            round((stats["healthy_prs"] / stats["total_prs"]) * 100, 1)
                        ...
                        "Total Resolved Conversations": stats[
                            "total_resolved_conversations"
                        ],
                        "Total Unresolved Conversations": stats[
                            "total_unresolved_conversations"
                        ],
                        ...
                        "Check Success Rate": (
                            round(
                                (
                                    stats["passed_checks"]
                                    / (stats["passed_checks"] + stats["failed_checks"])
                                )
                                * 100,
                                1,
                            )
                            if (stats["passed_checks"] + stats["failed_checks"]) > 0
                            else "N/A"
                        ),
                    }
                )
            ...
            with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
                ...
                self._format_excel_sheet(
                    writer.sheets["Contributor Metrics"], df, writer.book
                )

                # Write summary data
                summary_df.to_excel(
                    writer, sheet_name="Contributor Summary", index=False
                )
                self._format_summary_sheet(
                    writer.sheets["Contributor Summary"], summary_df, writer.book
                )
    ...
    def _format_excel_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            for idx, col in enumerate(dataframe.columns):
                max_len = (
                    max(dataframe[col].astype(str).apply(len).max(), len(str(col))) + 2
                )
                ...
                if "Date" in col:
                    ...
                    worksheet.conditional_format(
                        ...
                        idx,
                        {
                            ...
                            "format": (
                                health_format_good
                                if not col == "Is Breaking Change"
                                else health_format_bad
                            ),
                        },
                    )
                    worksheet.conditional_format(
...
def parse_arguments():
    ...
    parser.add_argument(
        "--repos-file", help="Path to file containing repository names (optional)"
    )
    parser.add_argument(
        "--token-file", help="Path to file containing GitHub token (optional)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="PR health threshold in days (default: 7)",
    )
    parser.add_argument(
        "--label-threshold",
        type=int,
        default=2,
        help="Maximum labels threshold (default: 2)",
    )
    ...
    try:
        ...
        if args.pr_threshold:
            ...
            reporter.logger.info(
                f"PR health threshold set to {reporter.pr_threshold_days} days"
            )
        ...
        if args.label_threshold:
            ...
            reporter.logger.info(
                f"Label threshold set to {reporter.max_labels_threshold}"
            )
        ...
        if not headers:
            reporter.logger.error(
                "Failed to authenticate with GitHub. Check token or permissions."
            )
            sys.exit(1)
        ...
        if args.repos_file and os.path.exists(args.repos_file):
            ...
            reporter.logger.info(
                f"Loaded {len(repositories)} repositories from {args.repos_file}"
            )
        else:
        ...
        if not repositories:
            reporter.logger.error(
                "No repositories found. Check organization name and permissions."
            )
            sys.exit(1)
        ...
        output_dir = (
            args.output_dir
            if args.output_dir
            else f'reports_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )
        ...
        for i, repo in enumerate(repositories, 1):
            try:
                ...
                metrics = reporter.fetch_pr_data(
                    headers, full_repo, start_date, end_date
                )
                if metrics:
        ...
        # Generate reports
        if all_metrics:
            reporter.generate_pr_activity_report(
                all_metrics, output_dir, all_repositories
            )
            reporter.generate_contributor_report(
                all_metrics,
                output_dir,
            ...
            total_prs = sum(
                metrics["stats"]["total_prs"] for metrics in all_metrics.values()
            )
            healthy_prs = sum(
                metrics["stats"]["healthy_prs"] for metrics in all_metrics.values()
            )
            unhealthy_prs = sum(
                metrics["stats"]["unhealthy_prs"] for metrics in all_metrics.values()
            )
            unhealthy_duration = sum(
                metrics["stats"]["unhealthy_due_to_duration"]
                for metrics in all_metrics.values()
            )
            unhealthy_labels = sum(
                metrics["stats"]["unhealthy_due_to_labels"]
                for metrics in all_metrics.values()
            )
            total_passed_checks = sum(
                metrics["stats"]["total_passed_checks"]
                for metrics in all_metrics.values()
            )
            total_failed_checks = sum(
                metrics["stats"]["total_failed_checks"]
                for metrics in all_metrics.values()
            )
            total_rc_versions = sum(
                metrics["stats"]["total_rc_versions"]
                for metrics in all_metrics.values()
            )
            total_npd_versions = sum(
                metrics["stats"]["total_npd_versions"]
                for metrics in all_metrics.values()
            )
            total_stable_versions = sum(
                metrics["stats"]["total_stable_versions"]
                for metrics in all_metrics.values()
            )
            ...
            print(
                f"PR Stats: {total_prs} total PRs, {healthy_prs} healthy, {unhealthy_prs} needs attention"
            )
            print(
                f"Needs Attention Breakdown: {unhealthy_duration} due to duration, {unhealthy_labels} due to label count"
            )
            print(
                f"Check Status: {total_passed_checks} passed checks, {total_failed_checks} failed checks"
            )
            print(
                f"Version Types: {total_rc_versions} RC, {total_npd_versions} NPD, {total_stable_versions} Stable"
            )
            print(f"Reports are available in: {output_dir}")
        else:
            reporter.logger.error(
                "No metrics data collected. Reports could not be generated."
            )
            sys.exit(1)
