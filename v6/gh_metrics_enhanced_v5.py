...
class GitHubMetricsReporter:
    ...
    Now with branch metrics tracking for contributors and stale branches detection.
    Also includes team information for contributors and marks non-contributors.
    """
    ...
    def __init__(self):
        ...
        # Branch staleness threshold (in days)
        self.branch_stale_threshold = 30
        self._setup_logging()
    ...
    def fetch_repository_branches(self, headers, repo):
        """
        Fetch all branches for a repository, including their last commit dates and authors.
        ...
        Returns:
            dict: Dictionary of branch information including last commit data and authors
        """
        try:
            self.logger.debug(f"Fetching branches for {repo}")
            ...
            while True:
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        if retry_count >= max_retries:
                            self.logger.error(f"Failed to fetch branches after {max_retries} retries: {str(e)}")
                            raise
                        self.logger.warning(f"Retry {retry_count}/{max_retries} for branches: {str(e)}")
                        time.sleep(2)
                
                if response.status_code != 200:
                    self.logger.error(f"Failed to fetch branches for {repo}: {response.status_code}")
                ...
                for branch in page_branches:
                    ...
                    # Fetch the commit details to get date and author
                    if last_commit_sha:
                        commit_info = self.fetch_commit_details(headers, repo, last_commit_sha)
                        branches[branch_name] = commit_info
                    else:
            ...
            self.logger.info(f"Found {len(branches)} branches in {repo}")
            return branches
            
        except Exception as e:
            self.logger.error(f"Error fetching branches for {repo}: {str(e)}")
            return {}

    def fetch_commit_details(self, headers, repo, commit_sha):
        """
        ...
        Returns:
            dict: Commit information including date and author
        """
        try:
            # Initialize default return value in case of any error
            default_result = {
                'last_commit_date': None,
                'last_commit_author': 'Unknown',
            ...
            if not headers or not repo or not commit_sha:
                self.logger.error(f"Missing required input for fetch_commit_details: headers={bool(headers)}, repo={repo}, commit_sha={commit_sha}")
                return default_result
                
            # Make API request with error handling
            try:
                response = requests.get(
                    f'{self.base_url}/repos/{repo}/commits/{commit_sha}',
                    headers=headers,
                    timeout=30
                )
            except requests.exceptions.RequestException as e:
                self.logger.error(f"Request exception when fetching commit {commit_sha}: {str(e)}")
                return default_result
            
            # Check response status
            if response.status_code != 200:
                self.logger.error(f"Failed to fetch commit details for {commit_sha}: {response.status_code}")
                return default_result
                
            # Process response data with careful error handling
            try:
                commit_data = response.json()
                
                # Check for required fields with safe access
                commit_info = commit_data.get('commit', {}) if commit_data else {}
                author_info = commit_info.get('author', {}) if commit_info else {}
                committer_info = commit_info.get('committer', {}) if commit_info else {}
                
                # Get commit date (use committer date as it's when the commit was actually applied)
                commit_date = None
                if committer_info and 'date' in committer_info:
                    try:
                        ...
                        self.logger.warning(f"Error parsing commit date for {commit_sha}: {str(e)}")
                
                # Get the author's login (username) with fallbacks
                author_login = None
                if commit_data and 'author' in commit_data and commit_data['author']:
                    author_login = commit_data['author'].get('login')
                
                if not author_login and author_info:
                    author_login = author_info.get('name')
                ...
                self.logger.error(f"Error parsing commit data for {commit_sha}: {str(e)}")
                return default_result
                
        except Exception as e:
            self.logger.error(f"Error fetching commit details for {commit_sha}: {str(e)}")
            return {
                'last_commit_date': None,
                'last_commit_author': 'Unknown',
                'last_commit_sha': commit_sha
    ...
    def is_branch_stale(self, branch_info, current_date, stale_days_threshold=30):
        """
        Determine if a branch is stale based on its last commit date.
        
        Args:
            branch_info (dict): Branch information including last commit date
            current_date (datetime): Current date for comparison
            stale_days_threshold (int): Number of days after which a branch is considered stale
            
        Returns:
            bool: True if the branch is stale, False otherwise
        """
        # If no last commit date, consider it stale
        if not branch_info.get('last_commit_date'):
            return True
        
        # Calculate days since last commit
        days_since_commit = (current_date - branch_info['last_commit_date']).days
        
        # Branch is stale if last commit is older than threshold
        return days_since_commit > stale_days_threshold

    def analyze_repository_branches(self, headers, repo, stale_days_threshold=30):
        """
        Analyze branches for a repository to determine active and stale branches by contributor.
        ...
        Returns:
            dict: Dictionary containing branch metrics
        """
        try:
            self.logger.info(f"Analyzing branches for {repo}")
            
            # Get current date
            current_date = datetime.now(self.utc)
            
            # Fetch branches
            branches = self.fetch_repository_branches(headers, repo)
            ...
            branch_metrics = {
                ...
                'stale_branches': 0,
                'active_branches': 0,
                'contributor_branches': {},  # Will store count of branches by contributor
                'branch_details': []  # Will store detailed branch information
            }
            
            # Analyze each branch
            for branch_name, branch_info in branches.items():
                # Check if branch is stale
                is_stale = self.is_branch_stale(branch_info, current_date, stale_days_threshold)
                
                # Get branch author
                author = branch_info.get('last_commit_author', 'Unknown')
                
                # Add to contributor branches count
                if author not in branch_metrics['contributor_branches']:
                    branch_metrics['contributor_branches'][author] = {
                        'total': 0,
                        'active': 0,
                        'stale': 0
                    }
                
                branch_metrics['contributor_branches'][author]['total'] += 1
                
                # Update stale/active counts
                if is_stale:
                    branch_metrics['stale_branches'] += 1
                    branch_metrics['contributor_branches'][author]['stale'] += 1
                else:
                    branch_metrics['active_branches'] += 1
                    branch_metrics['contributor_branches'][author]['active'] += 1
                
                # Add to branch details
                days_since_commit = None
                if branch_info.get('last_commit_date'):
                    days_since_commit = (current_date - branch_info['last_commit_date']).days
                    
                branch_metrics['branch_details'].append({
                    'name': branch_name,
            ...
            self.logger.info(f"Branch analysis for {repo}: {branch_metrics['total_branches']} total, "
                           f"{branch_metrics['active_branches']} active, {branch_metrics['stale_branches']} stale")
            
            return branch_metrics
            
        except Exception as e:
            self.logger.error(f"Error analyzing branches for {repo}: {str(e)}")
            return {
                'total_branches': 0,
                'stale_branches': 0,
                'active_branches': 0,
    ...
    def get_contributor_team_details(self, headers, org, username):
        """
        Get detailed team information for a contributor in an organization.
        
        Args:
            headers (dict): API headers
            org (str): Organization name
            username (str): Contributor's username
            
        Returns:
            dict: Dictionary with team information including team names and roles
        """
        try:
            self.logger.debug(f"Fetching team details for {username} in {org}")
            ...
            # If not a member, return default values
            if membership_response.status_code != 200:
                return team_details
                
            # User is a member, update details
            team_details['is_member'] = True
            membership_data = membership_response.json()
            team_details['role'] = membership_data.get('role', 'member')
            ...
            if teams_response.status_code != 200:
                self.logger.error(f"Failed to fetch teams for org {org}: {teams_response.status_code}")
                return team_details
                
            teams = teams_response.json()
            
            # Check team membership for each team
            user_teams = []
            for team in teams:
                ...
                if team_membership_response.status_code == 200:
                    team_info = {
                        ...
                        'role': team_membership_response.json().get('role', 'member')
                    }
                    user_teams.append(team_info)
            
            team_details['teams'] = user_teams
            
            # Set primary team (first team, or None if no teams)
            if user_teams:
                team_details['primary_team'] = user_teams[0]['name']
                
            return team_details
            
        except Exception as e:
            self.logger.error(f"Error fetching team details for {username}: {str(e)}")
            return {
                'is_member': False,
                'teams': [],
                'primary_team': None,
    ...
    def fetch_pr_data(self, headers, repo, start_date, end_date):
        ...
        - Team information for contributors
        """
        try:
            ...
            # Add branch metrics to the repo metrics
            branch_metrics = self.analyze_repository_branches(headers, repo, stale_days_threshold=self.branch_stale_threshold)
            metrics['branch_metrics'] = branch_metrics
            
            # Get org name from repo
            org_name = repo.split('/')[0]

            # Collect unique contributors to get team details
            all_contributors = set()
            for pr in metrics['pull_requests']:
                all_contributors.add(pr['author'])

            # Get team details for all contributors
            contributor_teams = {}
            for contributor in all_contributors:
                team_details = self.get_contributor_team_details(headers, org_name, contributor)
                contributor_teams[contributor] = team_details

            # Add team details to metrics
            metrics['contributor_teams'] = contributor_teams
    ...
    def generate_pr_activity_report(self, all_metrics, output_dir, all_repositories=None):
        ...
        - Branch metrics and team information
        ...
        try:
            ...
            for repo, metrics in all_metrics.items():
                ...
                if 'branch_metrics' in metrics:
                    repo_summary['Total Branches'] = metrics['branch_metrics']['total_branches']
                    repo_summary['Active Branches'] = metrics['branch_metrics']['active_branches']
                    repo_summary['Stale Branches'] = metrics['branch_metrics']['stale_branches']
                ...
                for pr in metrics['pull_requests']:
                    ...
                    # Get team information for PR author
                    team_info = {}
                    if 'contributor_teams' in metrics and pr['author'] in metrics['contributor_teams']:
                        team_info = metrics['contributor_teams'][pr['author']]
                    
                    # Extract team names and roles
                    team_names = [team['name'] for team in team_info.get('teams', [])]
                    primary_team = team_info.get('primary_team', 'No Team')
                    org_role = team_info.get('role', 'Not Member')
                    ...
                    record = {
                        ...
                        'Author': pr['author'],
                        'Team': primary_team,
                        'Teams': ', '.join(team_names) if team_names else 'No Team',
                        'Org Role': org_role,
            ...
            if all_repositories:
                ...
                for repo in all_repositories:
                    if repo not in processed_repos:
                        ...
                        # Try to add branch metrics if available
                        if repo in all_metrics and 'branch_metrics' in all_metrics[repo]:
                            branch_metrics = all_metrics[repo]['branch_metrics']
                            repo_summary['Total Branches'] = branch_metrics['total_branches']
                            repo_summary['Active Branches'] = branch_metrics['active_branches']
                            repo_summary['Stale Branches'] = branch_metrics['stale_branches']
    ...
    def generate_contributor_report(self, all_metrics, output_dir, all_contributors=None, headers=None, start_date=None, end_date=None):
        ...
        - Health indicators
        - PR comment metrics (moved from PR activity report)
        - Average passed/failed checks per PR
        - PR coverage metrics by contributor
        - Branch metrics (total, active, stale branches)
        - Team information (teams, primary team, org role)
        - Tracks non-contributors with "NC" values
        ...
        """
        try:
            # Convert all_contributors to a set if it's provided as a list
            if all_contributors is not None and isinstance(all_contributors, list):
                all_contributors = set(all_contributors)
            elif all_contributors is None:
                all_contributors = set()
            ...
            # Add tracking for PR-level pass/fail checks and comment metrics
            contributor_pr_checks = {}
            ...
            for repo, metrics in all_metrics.items():
                ...
                # Initialize PR coverage tracking by contributor
                for pr in metrics['pull_requests']:
                    author = pr['author']
                    
                    # Initialize contributor PR checks tracking
                    if author not in contributor_pr_checks:
                        contributor_pr_checks[author] = {
                            'total_prs': 0,
                            'total_passed_checks': 0,
                            'total_failed_checks': 0,
                        }
                    
                    contributor_pr_checks[author]['total_prs'] += 1
                    contributor_pr_checks[author]['total_passed_checks'] += pr['passed_checks']
                    contributor_pr_checks[author]['total_failed_checks'] += pr['failed_checks']
                    
                    # Initialize feature/fix PR coverage tracking
                    if author not in contributor_coverage:
                        contributor_coverage[author] = {
                            'feature_fix_prs': 0,
                            'with_examples': 0,
                            'with_tests': 0, 
                            'with_integration_tests': 0
                        }
                    
                    # Update feature/fix PR coverage data
                    if pr.get('is_feat_fix_pr', False):
                        contributor_coverage[author]['feature_fix_prs'] += 1
                        if pr.get('has_examples', False):
                            contributor_coverage[author]['with_examples'] += 1
                        if pr.get('has_tests', False):
                            contributor_coverage[author]['with_tests'] += 1
                        if pr.get('has_integration_tests', False):
                            contributor_coverage[author]['with_integration_tests'] += 1
                    ...
                    if author not in contributors:
                        contributors[author] = {
                            ...
                            # New fields for per-PR metrics
                            'avg_passed_checks_per_pr': 0,
                            'avg_failed_checks_per_pr': 0,
                            'total_commit_passed_checks': 0,
                            'total_commit_failed_checks': 0,
                            # New branch metrics
                            'total_branches': 0,
                            'active_branches': 0,
                            'stale_branches': 0,
                    ...
                    # Track commit-level pass/fail checks - added for new requirements
                    for commit in pr['commits']:
                        stats['total_commit_passed_checks'] += commit.get('passed_checks', 0)
                        stats['total_commit_failed_checks'] += commit.get('failed_checks', 0)
                ...
                # Add team information to contributor records
                for author, stats in contributors.items():
                    # Get team information from repo metrics
                    if 'contributor_teams' in metrics and author in metrics['contributor_teams']:
                        team_info = metrics['contributor_teams'][author]
                        team_names = [team['name'] for team in team_info.get('teams', [])]
                        primary_team = team_info.get('primary_team', 'No Team')
                        
                        # Add to contributor stats
                        stats['team_names'] = ', '.join(team_names) if team_names else 'No Team'
                        stats['primary_team'] = primary_team
                        stats['org_role'] = team_info.get('role', 'Not Member')
                
                # Add branch metrics to contributor data
                if 'branch_metrics' in metrics:
                    for author, branch_stats in metrics['branch_metrics']['contributor_branches'].items():
                        # Check if this contributor is already in our tracking
                        if author in contributors:
                            contributors[author]['total_branches'] = branch_stats['total']
                            contributors[author]['active_branches'] = branch_stats['active']
                            contributors[author]['stale_branches'] = branch_stats['stale']
                        # If not in our tracking (might be a contributor who only has branches, no PRs)
                        else:
                            ...
                            contributors[author] = {
                                ...
                                'avg_passed_checks_per_pr': 0,
                                'avg_failed_checks_per_pr': 0,
                                'total_branches': branch_stats['total'],
                                'active_branches': branch_stats['active'],
                                'stale_branches': branch_stats['stale'],
                                'team_names': ', '.join(team_names) if team_names else 'No Team',
                                'primary_team': primary_team,
                                'org_role': org_role
                            }
                            
                            # Add to contributor tracking
                ...
                for author, stats in contributors.items():
                    ...
                    # Calculate average checks per PR - added for new requirements
                    if stats['total_prs'] > 0:
                        stats['avg_passed_checks_per_pr'] = round(stats['passed_checks'] / stats['total_prs'], 2)
                        stats['avg_failed_checks_per_pr'] = round(stats['failed_checks'] / stats['total_prs'], 2)
                    
                    # Initialize branch metrics for contributors who don't have any
                    if 'total_branches' not in stats:
                        stats['total_branches'] = 0
                        stats['active_branches'] = 0
                        stats['stale_branches'] = 0
                    ...
                    if author not in all_contributors_data:
                        all_contributors_data[author] = {
                            ...
                            'total_unresolved_conversations': 0,
                            'breaking_change_prs': 0,
                            # New fields for commit level checks and PR averages
                            'total_commit_passed_checks': 0,
                            'total_commit_failed_checks': 0,
                            'avg_passed_checks_per_pr': 0,
                            'avg_failed_checks_per_pr': 0,
                            # Feature/fix PR coverage fields
                            'feature_fix_prs': 0,
                            'with_examples': 0,
                            'with_tests': 0,
                            'with_integration_tests': 0,
                            ...
                            'stale_branches': 0,
                            # Team information
                            'team_names': stats.get('team_names', 'No Team'),
                            'primary_team': stats.get('primary_team', 'No Team'),
                            'org_role': stats.get('org_role', 'Not Member')
                        }
                    ...
                    # Update commit-level check stats
                    all_contributors_data[author]['total_commit_passed_checks'] += stats['total_commit_passed_checks']
                    all_contributors_data[author]['total_commit_failed_checks'] += stats['total_commit_failed_checks']
                    
                    # Update branch metrics
                    all_contributors_data[author]['total_branches'] += stats.get('total_branches', 0)
                    all_contributors_data[author]['active_branches'] += stats.get('active_branches', 0)
                    all_contributors_data[author]['stale_branches'] += stats.get('stale_branches', 0)
                    
                    # Update team information (use the latest team info)
                    if stats.get('team_names', 'No Team') != 'No Team':
                        all_contributors_data[author]['team_names'] = stats['team_names']
                        all_contributors_data[author]['primary_team'] = stats['primary_team']
                        all_contributors_data[author]['org_role'] = stats['org_role']
            
            # Add feature/fix PR coverage data to all_contributors_data
            for author, coverage in contributor_coverage.items():
                if author in all_contributors_data:
                    all_contributors_data[author]['feature_fix_prs'] = coverage['feature_fix_prs']
                    all_contributors_data[author]['with_examples'] = coverage['with_examples']
                    all_contributors_data[author]['with_tests'] = coverage['with_tests']
                    all_contributors_data[author]['with_integration_tests'] = coverage['with_integration_tests']
            
            # Calculate PR-level averages for contributor summary
            for author, checks in contributor_pr_checks.items():
                if author in all_contributors_data and checks['total_prs'] > 0:
                    all_contributors_data[author]['avg_passed_checks_per_pr'] = round(
                        checks['total_passed_checks'] / checks['total_prs'], 2
                    )
                    all_contributors_data[author]['avg_failed_checks_per_pr'] = round(
                        checks['total_failed_checks'] / checks['total_prs'], 2
                    )
            ...
            if headers and start_date and end_date:
                for repo in all_metrics:
                    ...
                    for author, stats in repo_contributors.items():
                        ...
                        # Get team information
                        team_info = {}
                        if 'contributor_teams' in all_metrics.get(full_repo, {}) and author in all_metrics[full_repo]['contributor_teams']:
                            team_info = all_metrics[full_repo]['contributor_teams'][author]
                        else:
                            # Try to fetch team details
                            team_info = self.get_contributor_team_details(headers, org_name, author)
                        ...
                        if author not in all_contributors_data:
                            ...
                            # Check if this contributor has branch metrics
                            if 'branch_metrics' in all_metrics.get(full_repo, {}) and author in all_metrics[full_repo]['branch_metrics'].get('contributor_branches', {}):
                                branch_stats = all_metrics[full_repo]['branch_metrics']['contributor_branches'][author]
                                all_contributors_data[author]['total_branches'] = branch_stats['total']
                                all_contributors_data[author]['active_branches'] = branch_stats['active']
                                all_contributors_data[author]['stale_branches'] = branch_stats['stale']
                            ...
                            contributor_data.append({
                                ...
                                'avg_passed_checks_per_pr': 0,
                                'avg_failed_checks_per_pr': 0,
                                'total_branches': all_contributors_data[author]['total_branches'],
                                'active_branches': all_contributors_data[author]['active_branches'],
                                'stale_branches': all_contributors_data[author]['stale_branches'],
                                'team_names': all_contributors_data[author]['team_names'],
                                'primary_team': all_contributors_data[author]['primary_team'],
                                'org_role': all_contributors_data[author]['org_role']
                            })
            ...
            if all_contributors:
                ...
                for contributor in all_contributors:
                    if contributor not in processed_contributors:
                        ...
                        team_info = {}
                        for repo, metrics in all_metrics.items():
                            if 'contributor_teams' in metrics and contributor in metrics['contributor_teams']:
                                team_info = metrics['contributor_teams'][contributor]
                                break
                        ...
                        contributor_data.append({
                            ...
                            'npd_versions': 'NC',
                            'stable_versions': 'NC',
                            'total_reviewer_comments': 'NC',
                            'total_approver_comments': 'NC',
                            'total_resolved_conversations': 'NC',
                            'total_unresolved_conversations': 'NC',
                            'breaking_change_prs': 'NC',
                            'first_commit_date': None,
                            'last_commit_date': None,
                            'active_days': 'NC',
                            'avg_commits_per_day': 'NC',
                            'total_commit_passed_checks': 'NC',
                            'total_commit_failed_checks': 'NC',
                            'avg_passed_checks_per_pr': 'NC',
                            'avg_failed_checks_per_pr': 'NC',
                            'total_branches': 'NC',
                            'active_branches': 'NC',
                            'stale_branches': 'NC',
                            'team_names': ', '.join(team_names) if team_names else 'No Team',
            ...
            for author, stats in all_contributors_data.items():
                # Calculate coverage percentages for coverage metrics
                examples_percentage = 0
                tests_percentage = 0
                integration_tests_percentage = 0
                
                if stats['feature_fix_prs'] != 'NC' and stats['feature_fix_prs'] > 0:
                    examples_percentage = round((stats['with_examples'] / stats['feature_fix_prs']) * 100, 1)
                    tests_percentage = round((stats['with_tests'] / stats['feature_fix_prs']) * 100, 1)
                    integration_tests_percentage = round((stats['with_integration_tests'] / stats['feature_fix_prs']) * 100, 1)
                elif stats['feature_fix_prs'] == 'NC':
                    examples_percentage = 'NC'
                    tests_percentage = 'NC'
                    integration_tests_percentage = 'NC'
                
                # Calculate check success rate
                check_success_rate = 'NC'
                if stats['passed_checks'] != 'NC' and stats['failed_checks'] != 'NC':
                    total_checks = stats['passed_checks'] + stats['failed_checks']
                    if total_checks > 0:
                        check_success_rate = round((stats['passed_checks'] / total_checks) * 100, 1)
                
                # Calculate health percentage
                health_percentage = 0
                health_ratio = '0/0'
                if stats['total_prs'] != 'NC' and stats['healthy_prs'] != 'NC' and stats['total_prs'] > 0:
                    health_percentage = round((stats['healthy_prs'] / stats['total_prs']) * 100, 1)
                    health_ratio = f"{stats['healthy_prs']}/{stats['total_prs']}"
                elif stats['total_prs'] == 'NC':
                    health_percentage = 'NC'
                    health_ratio = 'NC'
                
                summary_entry = {
                    'Contributor': author,
                    'Team Names': stats.get('team_names', 'No Team'),
                    'Primary Team': stats.get('primary_team', 'No Team'),
                    'Org Role': stats.get('org_role', 'Not Member'),
                    'Repositories': len(stats['repositories']) if isinstance(stats['repositories'], set) else 'NC',
                    'Repository List': ', '.join(list(stats['repositories'])[:3]) + ('...' if isinstance(stats['repositories'], set) and len(stats['repositories']) > 3 else '') if isinstance(stats['repositories'], set) else 'NC',
                    ...
                    'Total Commit Passed Checks': stats['total_commit_passed_checks'],
                    'Total Commit Failed Checks': stats['total_commit_failed_checks'],
                    'Avg Passed Checks per PR': stats['avg_passed_checks_per_pr'],
                    'Avg Failed Checks per PR': stats['avg_failed_checks_per_pr'],
                    'Check Success Rate': check_success_rate,
                    'Feature/Fix PRs': stats['feature_fix_prs'],
                    'With Examples': stats['with_examples'],
                    'With Tests': stats['with_tests'],
                    'With Integration Tests': stats['with_integration_tests'],
                    'Examples Coverage (%)': examples_percentage,
                    'Tests Coverage (%)': tests_percentage,
                    'Integration Tests Coverage (%)': integration_tests_percentage,
                    'Total Branches': stats['total_branches'],
                    'Active Branches': stats['active_branches'],
                    'Stale Branches': stats['stale_branches']
                }
    ...
    def _format_excel_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            # Format for non-contributing values
            nc_format = workbook.add_format({
                'bg_color': '#F0F0F0',  # Light gray
                'font_color': '#888888',  # Dark gray
                'align': 'center'
            })
            ...
            for idx, col in enumerate(dataframe.columns):
                ...
                elif col in ['Days Open', 'Avg Days to Merge', 'PR Days Open', 'Avg Commits Per Day', 'Active Days', 'Avg Passed Checks per PR', 'Avg Failed Checks per PR']:
                    worksheet.set_column(idx, idx, max(8, max_len), decimal_format)
                elif 'Percentage' in col or 'Rate' in col or 'Coverage' in col:
                    worksheet.set_column(idx, idx, max(8, max_len), percentage_format)
                elif any(word in col for word in ['Count', 'Added', 'Deleted', 'Number', 'Total', 'PRs', 'Lines', 'Requests', 'Changes', 'Commits', 'Checks', 'Versions', 'Comments', 'Conversations', 'Branches']):
                    worksheet.set_column(idx, idx, max(8, max_len), number_format)
                elif col in ['Title', 'Commit Message', 'Labels', 'Approver Comments', 'Changed Files', 'Health Reasons', 'Team', 'Teams', 'Team Names', 'Primary Team']:
            ...
            # Apply NC formatting to non-contributing values
            for idx, col in enumerate(dataframe.columns):
                for row_idx, value in enumerate(dataframe[col], 1):
                    if str(value) == 'NC':
                        worksheet.write(row_idx, idx, 'NC', nc_format)
    ...
    def _format_summary_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            for idx, col in enumerate(dataframe.columns):
                ...
                if 'Percentage' in col or 'Rate' in col or 'Coverage' in col:
                ...
                elif any(word in col for word in ['Total', 'Count', 'PRs', 'Checks', 'Commits', 'Repositories', 'Versions', 'Comments', 'Conversations', 'Examples', 'Tests', 'Branches']):
                    worksheet.set_column(idx, idx, max(8, max_len), number_format)
                
                elif 'Repository' in col or 'Contributor' in col:
                    worksheet.set_column(idx, idx, max(20, max_len), text_format)
                
                elif 'Team' in col or 'Org Role' in col:
                    worksheet.set_column(idx, idx, max(15, max_len), text_format)
                
                elif 'List' in col:
                    worksheet.set_column(idx, idx, min(50, max_len), text_format)
                
                elif 'Duration' in col or 'Avg' in col:
...
def parse_arguments():
    ...
    parser.add_argument('--branch-stale-threshold', type=int, default=30, help='Days after which a branch is considered stale (default: 30)')
...
def main():
    ...
    try:
        ...
        # Set branch stale threshold from command line if provided
        if args.branch_stale_threshold:
            reporter.branch_stale_threshold = args.branch_stale_threshold
            reporter.logger.info(f"Branch stale threshold set to {reporter.branch_stale_threshold} days")
        ...
        for i, repo in enumerate(repositories, 1):
            try:
                ...
                if metrics:
                    ...
                    # Branch metrics
                    total_branches = metrics['branch_metrics']['total_branches']
                    active_branches = metrics['branch_metrics']['active_branches']
                    stale_branches = metrics['branch_metrics']['stale_branches']
                    
                    # Collect all contributors from this repo
                    for pr in metrics.get('pull_requests', []):
                        all_contributors.add(pr['author'])
                    
                    # Collect branch contributors
                    if 'branch_metrics' in metrics and 'contributor_branches' in metrics['branch_metrics']:
                        for author in metrics['branch_metrics']['contributor_branches'].keys():
                            all_contributors.add(author)
                    
                    # Collect team contributors
                    if 'contributor_teams' in metrics:
                        for author in metrics['contributor_teams'].keys():
                            all_contributors.add(author)
                    ...
                    reporter.logger.info(
                        ...
                        f"{total_branches} branches ({active_branches} active, {stale_branches} stale)"
                    )
        ...
        if all_metrics:
            ...
            reporter.generate_contributor_report(all_metrics, output_dir, all_contributors, headers, start_date, end_date)
            ...
            total_branches = sum(metrics['branch_metrics']['total_branches'] for metrics in all_metrics.values())
            active_branches = sum(metrics['branch_metrics']['active_branches'] for metrics in all_metrics.values())
            stale_branches = sum(metrics['branch_metrics']['stale_branches'] for metrics in all_metrics.values())
            
            # Count unique teams
            all_teams = set()
            for repo_metrics in all_metrics.values():
                if 'contributor_teams' in repo_metrics:
                    for team_info in repo_metrics['contributor_teams'].values():
                        for team in team_info.get('teams', []):
                            all_teams.add(team.get('name', ''))
            ...
            print(f"Branch Stats: {total_branches} total branches, {active_branches} active, {stale_branches} stale")
            print(f"Team Stats: {len(all_teams)} teams, {len(all_contributors)} contributors")
