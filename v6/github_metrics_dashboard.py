    ...
    def create_contributor_summary(self, data):
        ...
        try:
            ...
            if 'Health Percentage' in complete_stats.columns:
                complete_stats['Health Score'] = complete_stats['Health Percentage'].apply(lambda x: f"{x}%" if x != 'NC' else 'NC')

            # Create a health status indicator
            if 'Health Percentage' in complete_stats.columns:
                def health_status_indicator(percentage, total_prs=None):
                    # Handle non-contributing case
                    if percentage == 'NC' or (total_prs is not None and total_prs == 'NC'):
            ...
            if all(col in complete_stats.columns for col in ['Passed Checks', 'Failed Checks']):
                def calculate_success_rate(row):
                    if row['Passed Checks'] == 'NC' or row['Failed Checks'] == 'NC':
                        return 'NC'
                    elif (row['Passed Checks'] + row['Failed Checks']) > 0:
            ...
            with col2:
                ...
                if min_prs > 0:
                    ...
                    numeric_mask = complete_stats['Total PRs'] != 'NC'
                    complete_stats = complete_stats[
                        (~numeric_mask) | (numeric_mask & (complete_stats['Total PRs'] >= min_prs))
                    ]
            ...
            with col1:
                ...
                if 'Repositories' in complete_stats.columns:
                    ...
                    if min_repos > 0:
                        numeric_mask = complete_stats['Repositories'] != 'NC'
                        complete_stats = complete_stats[
                            (~numeric_mask) | (numeric_mask & (complete_stats['Repositories'] >= min_repos))
                        ]
            ...
            with col2:
                ...
                if min_commits > 0:
                    numeric_mask = complete_stats['Total Commits'] != 'NC'
                    complete_stats = complete_stats[
                        (~numeric_mask) | (numeric_mask & (complete_stats['Total Commits'] >= min_commits))
                    ]
            ...
            # Add third row of filters for branch metrics
            if any(col in complete_stats.columns for col in branch_cols):
                ...
                with col1:
                    ...
                    if 'Total Branches' in complete_stats.columns:
                        ...
                        if min_branches > 0:
                            numeric_mask = complete_stats['Total Branches'] != 'NC'
                            complete_stats = complete_stats[
                                (~numeric_mask) | (numeric_mask & (complete_stats['Total Branches'] >= min_branches))
                            ]
                ...
                with col2:
                    ...
                    if 'Active Branches' in complete_stats.columns:
                        ...
                        if min_active > 0:
                            numeric_mask = complete_stats['Active Branches'] != 'NC'
                            complete_stats = complete_stats[
                                (~numeric_mask) | (numeric_mask & (complete_stats['Active Branches'] >= min_active))
                            ]
                ...
                with col3:
                    ...
                    if 'Stale Branches' in complete_stats.columns:
                        ...
                        if min_stale > 0:
                            numeric_mask = complete_stats['Stale Branches'] != 'NC'
                            complete_stats = complete_stats[
                                (~numeric_mask) | (numeric_mask & (complete_stats['Stale Branches'] >= min_stale))
                            ]
            ...
            with col2:
                ...
                # Handle sorting with NC values
                if sort_column in complete_stats.columns:
                    # Create a mask for NC values
                    if not include_nc_in_sorting:
                        nc_mask = complete_stats[sort_column] == 'NC'
                        # Sort non-NC values
                        non_nc = complete_stats[~nc_mask].sort_values(by=sort_column, ascending=sort_ascending)
                        # Append NC values at the end
                        nc_values = complete_stats[nc_mask]
                        complete_stats = pd.concat([non_nc, nc_values])
                    else:
                        # Regular sort - NC values will be handled according to string comparison
