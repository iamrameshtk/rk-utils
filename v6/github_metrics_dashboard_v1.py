    ...
    def create_contributor_summary(self, data):
        ...
        try:
            ...
            if 'Health Percentage' in complete_stats.columns:
                # Safe conversion that handles 'NC' values
                complete_stats['Health Score'] = complete_stats['Health Percentage'].apply(
                    lambda x: f"{x}%" if x != 'NC' else 'NC'
                )

            # Create a health status indicator - fixed version
            if 'Health Percentage' in complete_stats.columns:
                def health_status_indicator(percentage, total_prs=None):
                    # Handle non-contributing case
                    if percentage == 'NC' or total_prs == 'NC':
                        return "⚪"  # White circle for NC
                
                    # Special case for contributors with no PRs
                    if total_prs is not None and (total_prs == 0 or total_prs == '0'):
                    ...
                    try:
                        ...
                        # If conversion fails, return white circle
                        return "⚪"
                ...
                if 'Total PRs' in complete_stats.columns:
                    ...
                    complete_stats['Health Status'] = complete_stats['Health Percentage'].apply(
                        lambda x: health_status_indicator(x)
                    )
            ...
            if all(col in complete_stats.columns for col in ['Passed Checks', 'Failed Checks']):
                def calculate_success_rate(row):
                    # Handle NC values
                    if row['Passed Checks'] == 'NC' or row['Failed Checks'] == 'NC':
                        return 'NC'
                
                    try:
                        # Convert to numbers if needed and do calculation
                        passed = float(row['Passed Checks']) if not isinstance(row['Passed Checks'], (int, float)) else row['Passed Checks']
                        failed = float(row['Failed Checks']) if not isinstance(row['Failed Checks'], (int, float)) else row['Failed Checks']
            ...
            display_cols = ['Contributor']
        
            # Add team columns if available
            team_cols = ['Primary Team', 'Team Names', 'Org Role']
            for col in team_cols:
                if col in complete_stats.columns:
                    display_cols.append(col)
        
            # Add standard metrics
            display_cols.extend(['Repositories', 'Total PRs', 'Total Commits'])
            ...
            with col2:
                # Filter by minimum PRs - Safe implementation
                min_prs = st.number_input("Minimum PRs:", min_value=0, value=0, key="min_prs")
                if min_prs > 0:
                    # Only filter numeric values, keep NC values
                    complete_stats = complete_stats[
                        (complete_stats['Total PRs'] == 'NC') | 
                        (complete_stats['Total PRs'].apply(lambda x: isinstance(x, (int, float)) and x >= min_prs))
                    ]

            with col3:
                # Filter by team - add proper error handling
                if 'Primary Team' in complete_stats.columns:
                    try:
                        # Get unique teams for filtering (include "All" and "No Team")
                        teams = ["All"]
                        team_values = complete_stats['Primary Team'].unique().tolist()
                        teams.extend(sorted([t for t in team_values if t != 'No Team']))
                        if 'No Team' in team_values:
                            teams.append('No Team')
                        
                        selected_team = st.selectbox("Filter by team:", teams, index=0, key="team_filter")
                        if selected_team != "All":
                            complete_stats = complete_stats[complete_stats['Primary Team'] == selected_team]
                    except Exception as e:
                        st.warning(f"Team filtering error: {str(e)}")
            
                # Fall back to health status filter if no team column
                elif 'Health Status' in complete_stats.columns:
                    try:
                        ...
                        st.warning(f"Health status filtering error: {str(e)}")

            # Add second row of filters - improved implementation for handling NC values
            col1, col2, col3 = st.columns(3)

            with col1:
                # Filter by repository count - safe implementation
                if 'Repositories' in complete_stats.columns:
                    ...
                    if min_repos > 0:
                        # Only keep NC values and values >= min_repos
                        complete_stats = complete_stats[
                            (complete_stats['Repositories'] == 'NC') | 
                            (complete_stats['Repositories'].apply(lambda x: isinstance(x, (int, float)) and x >= min_repos))
                        ]

            with col2:
                # Filter by minimum commits - safe implementation
                min_commits = st.number_input("Minimum Commits:", min_value=0, value=0, key="min_commits")
                if min_commits > 0:
                    # Only keep NC values and values >= min_commits
                    complete_stats = complete_stats[
                        (complete_stats['Total Commits'] == 'NC') | 
                        (complete_stats['Total Commits'].apply(lambda x: isinstance(x, (int, float)) and x >= min_commits))
                    ]

            with col3:
                # Filter to show only non-contributors
                show_nc_only = st.checkbox("Show only non-contributors (NC)", value=False, key="show_nc")
                if show_nc_only:
                    # Filter to only show rows where Total PRs is 'NC'
                    complete_stats = complete_stats[complete_stats['Total PRs'] == 'NC']

            # Add third row of filters for branch metrics - safe implementation
            if any(col in complete_stats.columns for col in branch_cols):
                ...
                with col1:
                    ...
                    if 'Total Branches' in complete_stats.columns:
                        ...
                        if min_branches > 0:
                            complete_stats = complete_stats[
                                (complete_stats['Total Branches'] == 'NC') | 
                                (complete_stats['Total Branches'].apply(lambda x: isinstance(x, (int, float)) and x >= min_branches))
                            ]
                ...
                with col2:
                    ...
                    if 'Active Branches' in complete_stats.columns:
                        ...
                        if min_active > 0:
                            complete_stats = complete_stats[
                                (complete_stats['Active Branches'] == 'NC') | 
                                (complete_stats['Active Branches'].apply(lambda x: isinstance(x, (int, float)) and x >= min_active))
                            ]
                ...
                with col3:
                    ...
                    if 'Stale Branches' in complete_stats.columns:
                        ...
                        if min_stale > 0:
                            complete_stats = complete_stats[
                                (complete_stats['Stale Branches'] == 'NC') | 
                                (complete_stats['Stale Branches'].apply(lambda x: isinstance(x, (int, float)) and x >= min_stale))
                            ]

            # Sort options - enhanced with better error handling
            ...
            with col1:
                ...
                # Add team sort option
                if 'Primary Team' in complete_stats.columns:
                    sort_options["Team (A-Z)"] = ("Primary Team", True)
                    sort_options["Team (Z-A)"] = ("Primary Team", False)
            ...
            with col2:
                # Add checkbox to include/exclude NC values in sorting
                include_nc_in_sorting = st.checkbox("Include NC values in sorting", value=False, key="include_nc_sort")
            
                # Handle sorting with NC values - improved error handling
                if sort_column in complete_stats.columns:
                    try:
                        if not include_nc_in_sorting:
                            # Create a mask for NC values - safer implementation
                            nc_mask = complete_stats[sort_column].astype(str) == 'NC'
                            # Separate NC and non-NC values
                            non_nc = complete_stats[~nc_mask].copy()
                            nc_values = complete_stats[nc_mask].copy()
                        
                            # Sort non-NC values safely
                            try:
                                # First try normal sorting
                                non_nc = non_nc.sort_values(by=sort_column, ascending=sort_ascending)
                            except Exception:
                                # If that fails, convert to string
                                non_nc = non_nc.sort_values(
                                    by=sort_column, 
                                    ascending=sort_ascending,
                                    key=lambda x: x.astype(str)
                                )
                            
                            # Combine sorted non-NC values with NC values at the end
                            complete_stats = pd.concat([non_nc, nc_values])
                        else:
                            # For sorting with NC values, convert to string for consistent behavior
                            complete_stats = complete_stats.sort_values(
                                by=sort_column, 
                                ascending=sort_ascending,
                                key=lambda x: x.astype(str)
                            )
                    except Exception as e:
                        st.warning(f"Sorting error: {str(e)}. Falling back to default sort.")
                        # Fall back to sorting by contributor name
                        complete_stats = complete_stats.sort_values(by="Contributor")
