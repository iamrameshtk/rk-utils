...
class GitHubMetricsReporter:
    ...
    def generate_pr_activity_report(self, all_metrics, output_dir):
        """
        Generate enhanced PR activity report with health indicators based on:
        - PR duration (> 7 days = unhealthy)
        - Label count (> 2 labels = unhealthy)
        ...
        try:
            ...
            pr_df['PR Health'] = pr_df['PR Health'].apply(lambda x: f"❌ {x}" if x == 'Unhealthy' else f"✅ {x}")
        ...
        try:
            ...
            for repo, metrics in all_metrics.items():
                ...
                for author, stats in contributors.items():
                    ...
                    all_contributors[author]['repositories'].add(repo)
                    all_contributors[author]['total_commits'] += stats['total_commits']
                    all_contributors[author]['total_prs'] += stats['total_prs']
                    all_contributors[author]['healthy_prs'] += stats['healthy_prs']
                    all_contributors[author]['unhealthy_prs'] += stats['unhealthy_prs']
                    all_contributors[author]['passed_checks'] += stats['passed_checks']
                    all_contributors[author]['failed_checks'] += stats['failed_checks']
                    all_contributors[author]['rc_versions'] += stats['rc_versions']
                    all_contributors[author]['npd_versions'] += stats['npd_versions']
                    all_contributors[author]['stable_versions'] += stats['stable_versions']
    ...
    def _format_excel_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            for idx, col in enumerate(dataframe.columns):
                ...
                elif any(word in col for word in ['Count', 'Added', 'Deleted', 'Number', 'Total', 'PRs', 'Lines', 'Requests', 'Changes', 'Commits', 'Checks', 'Versions']):
    ...
    def _format_summary_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            for idx, col in enumerate(dataframe.columns):
                ...
                elif any(word in col for word in ['Total', 'Count', 'PRs', 'Checks', 'Commits', 'Repositories', 'Versions']):
                    worksheet.set_column(idx, idx, max(8, max_len), number_format)
...
def main():
    ...
    try:
        ...
        # Generate reports
        if all_metrics:
            reporter.generate_pr_activity_report(all_metrics, output_dir)
            ...
            print(f"Unhealthy Breakdown: {unhealthy_duration} due to duration, {unhealthy_labels} due to label count")
