...
class GitHubMetricsDashboard:
    ...
    def calculate_contributor_review_metrics(self, pr_activity):
        """Calculate PR approvals and change requests given by each contributor."""
        try:
            # Initialize dictionaries to track metrics
            approvals_by_contributor = {}
            change_requests_by_contributor = {}
            
            # Check if required columns exist
            if 'Approvers' not in pr_activity.columns:
                st.warning("Approvers data not available in the PR activity report.")
                return None, None
            
            # Process each PR to count approvals
            for _, row in pr_activity.iterrows():
                # Process approvers
                approvers = row.get('Approvers', '')
                if isinstance(approvers, str) and approvers not in ('None', ''):
                    # Split approvers list - assuming they're comma-separated
                    approver_list = [a.strip() for a in approvers.split(',')]
                    for approver in approver_list:
                        if approver:  # Skip empty strings
                            if approver not in approvals_by_contributor:
                                approvals_by_contributor[approver] = 0
                            approvals_by_contributor[approver] += 1
                
                # Process change requests - look for different possible column names
                if 'Change Requests' in pr_activity.columns and isinstance(row.get('Change Requests'), (int, float)):
                    # If only count is available, we can't attribute to specific contributors
                    pass
                    
                elif 'Changes Status' in pr_activity.columns and 'Changes Requested By' in pr_activity.columns:
                    # If we have specific data on who requested changes
                    requestors = row.get('Changes Requested By', '')
                    if isinstance(requestors, str) and requestors not in ('None', ''):
                        requestor_list = [r.strip() for r in requestors.split(',')]
                        for requestor in requestor_list:
                            if requestor:  # Skip empty strings
                                if requestor not in change_requests_by_contributor:
                                    change_requests_by_contributor[requestor] = 0
                                change_requests_by_contributor[requestor] += 1
            
            # Convert dictionaries to dataframes
            approvals_df = pd.DataFrame({
                'Contributor': list(approvals_by_contributor.keys()),
                'PRs Approved': list(approvals_by_contributor.values())
            })
            
            change_requests_df = pd.DataFrame({
                'Contributor': list(change_requests_by_contributor.keys()),
                'Change Requests Given': list(change_requests_by_contributor.values())
            })
            
            return approvals_df, change_requests_df
            
        except Exception as e:
            st.error(f"Error calculating contributor review metrics: {str(e)}")
            return None, None
    ...
    def create_contributor_summary(self, data):
        ...
        try:
            ...
            # Calculate review metrics - PRs approved and change requests given by each contributor
            approvals_df, change_requests_df = self.calculate_contributor_review_metrics(pr_activity)
            
            # Add these metrics to contributor summary if available
            if approvals_df is not None:
                contributor_summary = pd.merge(
                    contributor_summary, 
                    approvals_df, 
                    on='Contributor', 
                    how='left'
                ).fillna(0)
                contributor_summary['PRs Approved'] = contributor_summary['PRs Approved'].astype(int)
            else:
                contributor_summary['PRs Approved'] = 0
                
            if change_requests_df is not None:
                contributor_summary = pd.merge(
                    contributor_summary, 
                    change_requests_df, 
                    on='Contributor', 
                ...
                contributor_summary['Change Requests Given'] = contributor_summary['Change Requests Given'].astype(int)
            else:
                contributor_summary['Change Requests Given'] = 0
            ...
            # New: Review Activity Metrics
            st.subheader("Review Activity Metrics")
            col1, col2 = st.columns(2)
            
            with col1:
                total_approvals = contributor_summary['PRs Approved'].sum()
                st.metric(
                    label="Total PRs Approved", 
                    value=total_approvals
                )
                
                if total_contributors > 0:
                    avg_approvals = round(total_approvals / total_contributors, 1)
                    st.metric(
                        label="Avg PRs Approved per Contributor", 
                        value=avg_approvals
                    )
            
            with col2:
                total_change_requests = contributor_summary['Change Requests Given'].sum()
                st.metric(
                    label="Total Change Requests Given", 
                    value=total_change_requests
                )
                
                if total_contributors > 0:
                    avg_change_requests = round(total_change_requests / total_contributors, 1)
                    st.metric(
                        label="Avg Change Requests per Contributor", 
                        value=avg_change_requests
                    )
            ...
            required_columns = ['Contributor', 'Repositories', 'Total PRs', 'Total Commits', 'Health Percentage', 'PRs Approved', 'Change Requests Given']
            ...
            display_columns = ['Contributor', 'Repositories', 'Total PRs', 'Total Commits', 'PRs Approved', 'Change Requests Given', 'Health Score']
            ...
            display_columns = ['Contributor', 'Total PRs', 'Healthy PRs', 'Unhealthy PRs', 'PRs Approved', 'Change Requests Given', 'Health Score']
            ...
            # Complete Contributor Statistics
            st.subheader("Complete Contributor Statistics")
            
            # Create expander for full stats to keep the UI clean
            with st.expander("View All Contributors", expanded=False):
                # Get all available columns that might be useful for a complete view
                all_columns = ['Contributor', 'Repositories', 'Total PRs', 'Healthy PRs', 'Unhealthy PRs', 
                              'PRs Approved', 'Change Requests Given', 'Health Score', 'Total Commits']
                
                # Add change requests and approvals columns
                for col in ['PRs Approved', 'Change Requests Given']:
                    if col not in contributor_summary.columns:
                        contributor_summary[col] = 0
                
                # Add version type counts if available
                for version_type in ['RC Versions', 'NPD Versions', 'Stable Versions']:
                    if version_type in contributor_summary.columns:
                        all_columns.append(version_type)
                
                # Add check status if available
                if all(col in contributor_summary.columns for col in ['Passed Checks', 'Failed Checks']):
                    all_columns.extend(['Passed Checks', 'Failed Checks'])
                    
                    # Add success rate calculation if not already present
                    if 'Check Success Rate' not in contributor_summary.columns:
                        def calculate_success_rate(row):
                            if (row['Passed Checks'] + row['Failed Checks']) > 0:
                                rate = round((row['Passed Checks'] / (row['Passed Checks'] + row['Failed Checks']) * 100), 1)
                                return f"{rate}%"
                        ...
                        contributor_summary['Check Success Rate'] = contributor_summary.apply(calculate_success_rate, axis=1)
                    
                    all_columns.append('Check Success Rate')
                
                # Make sure all display columns exist in the dataframe
                all_columns = [col for col in all_columns if col in contributor_summary.columns]
                
                # Allow sorting by different metrics
                sort_options = [col for col in all_columns if col != 'Check Success Rate' and col != 'Health Score']
                sort_by = st.selectbox("Sort By:", sort_options, index=sort_options.index('Total PRs') if 'Total PRs' in sort_options else 0)
                sort_ascending = st.checkbox("Ascending Order", value=False)
                
                # Sort and display full stats
                sorted_contributors = contributor_summary.sort_values(by=sort_by, ascending=sort_ascending)
                
                # Display with pagination or scrollable area to handle large number of contributors
                st.dataframe(sorted_contributors[all_columns], use_container_width=True, height=400)
                
                # Show summary statistics
                with st.columns(3)[1]:
                    st.write(f"Total: {len(sorted_contributors)} contributors")
