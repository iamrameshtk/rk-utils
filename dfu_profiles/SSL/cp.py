    ...
    try:
        ...
        if location:
            ...
            try:
                ...
                # If SSL error occurs, retry without verification
                logger.warning(f"SSL verification failed: {ssl_err}")
...
def create_compute_profile(project_id, location, instance_name, namespace, profile_name, profile_config, access_token):
    ...
    try:
        ...
        try:
            ...
            try:
                response = requests.put(url, headers=headers, data=json_payload, timeout=60, verify=verify_ssl)
            except requests.exceptions.SSLError as ssl_err:
                # If SSL error occurs, retry without verification
                logger.warning(f"SSL verification failed: {ssl_err}")
                logger.warning("Retrying request with SSL verification disabled")
