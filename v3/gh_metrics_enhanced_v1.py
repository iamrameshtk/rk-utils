...
class GitHubMetricsReporter:
    ...
    def __init__(self):
        ...
        self.branch_staleness_threshold = 30
        self._setup_logging()
    ...
    def fetch_contributor_branches(self, headers, repo, start_date, end_date):
        """
        Fetch branches created or updated by each contributor within the date range.
        """
        try:
            self.logger.debug(f"Fetching branch data for {repo} between {start_date.date()} and {end_date.date()}")

            # Track contributors and their branch counts
            ...
            while True:
                self.logger.debug(f"Fetching page {page} of branches for {repo}")
                ...
                # Handle 404 for repositories without branches or empty repositories
                if response.status_code == 404:
                    self.logger.info(f"No branches found for repository {repo} (404 response)")
                    return {}  # Return empty dict rather than raising an error
                ...
                for branch in branches:
                    try:
                        ...
                        commit_sha = branch.get('commit', {}).get('sha', '') if branch else ''
                        if not commit_sha:
                            continue
                    
                        # Get commit details to determine the committer and date
                        commit_response = requests.get(
                        f'{self.base_url}/repos/{repo}/commits/{commit_sha}',
                        headers=headers
                        )

                        # Handle 404 for commits that might have been deleted or force-pushed
                        if commit_response.status_code == 404:
                            self.logger.debug(f"Commit {commit_sha} not found for branch {branch_name} in {repo}")
                            continue                        
                        ...
                        if commit_response.status_code == 200:
                            ...
                            # Get the commit date safely
                            commit_obj = commit_data.get('commit', {})
                            committer_obj = commit_obj.get('committer', {}) if commit_obj else {}
                            commit_date_str = committer_obj.get('date', '') if committer_obj else ''
                            ...
                            if commit_date_str:
                                ...
                                days_since_last_commit = (end_date - commit_date).days

                                # Clear debug statement to verify calculation
                                self.logger.debug(f"Branch {branch_name}: Last commit was {days_since_last_commit} days ago")
                            
                                # Determine if branch is stale based on threshold
                                is_stale = days_since_last_commit > self.branch_staleness_threshold
                            
                                # Get committer username safely
                                committer_obj = commit_data.get('committer', {})
                                committer_login = committer_obj.get('login', '') if committer_obj else ''
                            
                                if committer_login:
                                    # Initialize contributor entry if not exists
                                    if committer_login not in contributor_branches:
                                        contributor_branches[committer_login] = {
                                            'active_branches': 0,
                                            'stale_branches': 0,
                                            'branches': []
                                        }
                                
                                    # Track the branch - store as string identifier, not a dictionary
                                    branch_info = {
                                        'name': branch_name,
                                        'last_commit': commit_date.isoformat(),  # Convert datetime to string
                                        'is_stale': is_stale
                                    }
                                
                                    contributor_branches[committer_login]['branches'].append(branch_info)
                                
                                    # Update branch counts
                                    if is_stale:
                                        contributor_branches[committer_login]['stale_branches'] += 1
                                    else:
                                        contributor_branches[committer_login]['active_branches'] += 1
                                        
                    except Exception as e:
                        self.logger.error(f"Error processing branch '{branch_name}' in {repo}: {str(e)}")
                        continue
                ...
                self.logger.info(f"Found branch data for {len(contributor_branches)} contributors in {repo}")
                return contributor_branches
        
        except Exception as e:
            self.logger.error(f"Error fetching branch data for {repo}: {str(e)}")
            return {}
    ...
    def generate_contributor_report(self, all_metrics, output_dir, all_contributors=None, headers=None, start_date=None, end_date=None):
        ...
        - Active and stale branches per contributor
        ...
        try:
            ...
            # Track branch metrics by contributor
            contributor_branch_metrics = {}
            
            # Fetch branch data if headers and dates are provided
            if headers and start_date and end_date:
                for repo in all_metrics:
                    try:
                        ...
                        # Get branch data
                        branch_data = self.fetch_contributor_branches(headers, full_repo, start_date, end_date)
                        
                        # Process branch data
                        for author, data in branch_data.items():
                            if author not in contributor_branch_metrics:
                                contributor_branch_metrics[author] = {
                                    'active_branches': 0,
                                    'stale_branches': 0
                                }
                            
                            contributor_branch_metrics[author]['active_branches'] += data['active_branches']
                            contributor_branch_metrics[author]['stale_branches'] += data['stale_branches']
                            
                            # Add contributor to tracking if not already there
                            if all_contributors is not None and author not in all_contributors:
                                all_contributors.add(author)
                            
                    except Exception as e:
                        self.logger.error(f"Error processing branch data for {repo}: {str(e)}")
            ...
            for repo, metrics in all_metrics.items():
                ...
                for author, stats in contributors.items():
                    ...
                    # Update commit-level check stats - added for new requirements
            ...
            for author, stats in all_contributors_data.items():
                ...
                # Add branch metrics if available for this contributor
                if author in contributor_branch_metrics:
                    summary_entry['Active Branches'] = contributor_branch_metrics[author]['active_branches']
                    summary_entry['Stale Branches'] = contributor_branch_metrics[author]['stale_branches']
                else:
                    summary_entry['Active Branches'] = 0
                    summary_entry['Stale Branches'] = 0
...
def parse_arguments():
    ...
    parser.add_argument('--branch-staleness', type=int, default=30, help='Branch staleness threshold in days (default: 30)')
...
def main():
    ...
    try:
        ...
        # Set branch staleness threshold if provided
        if args.branch_staleness:
            reporter.branch_staleness_threshold = args.branch_staleness
            reporter.logger.info(f"Branch staleness threshold set to {reporter.branch_staleness_threshold} days")
