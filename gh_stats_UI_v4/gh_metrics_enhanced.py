...
class GitHubMetricsReporter:
    ...
    def is_feat_or_fix_pr(self, pr_title):
        """
        Check if a PR title starts with 'feat:' or 'fix:' or contains these as prefixes.
        
        Args:
            pr_title (str): The PR title to check
            
        Returns:
            bool: True if the PR title matches the criteria, False otherwise
        """
        if not pr_title:
            return False
        ...
        prefixes = ['feat:', 'feat(', 'fix:', 'fix(']
        for prefix in prefixes:
            if title_lower.startswith(prefix):
                return True
        
        # Check for standalone words at the beginning
        if title_lower.startswith('feat ') or title_lower.startswith('fix '):
            return True
                ...
                for pr in prs:
                    try:
                        ...
                        if start_date <= created_at <= end_date:
                            ...
                            # Determine if this is a feat/fix PR
                            is_feat_fix = self.is_feat_or_fix_pr(pr['title'])
    ...
    def generate_contributor_report(self, all_metrics, output_dir, all_contributors=None):
        """
    ...
    def _format_excel_sheet(self, worksheet, dataframe, workbook):
        ...
        try:
            ...
            for idx, col in enumerate(dataframe.columns):
                ...
                elif col in ['Has Examples', 'Has Tests', 'Has Integration Tests']:
...
def main():
    ...
    try:
        ...
        if all_metrics:
            ...
            reporter.generate_contributor_report(all_metrics, output_dir, list(all_contributors))
