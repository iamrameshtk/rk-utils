            ...
            if any(col in contributor_summary.columns for col in branch_columns):
                ...
                if all(col in contributor_summary.columns for col in branch_columns):
                    branch_metrics['Active:Stale Ratio'] = (
                        ...
                        contributor_summary['Stale Branches'].apply(lambda x: 1 if x == 0 else x)
                    ).round(2)
                
                # Filter to show only contributors with branch activity
                has_branch_activity = branch_metrics[branch_columns[0]] > 0 if branch_columns[0] in branch_metrics.columns else False
                if any(has_branch_activity):
                    ...
                    with col1:
                        total_active = active_contributors[branch_columns[0]].sum() if branch_columns[0] in active_contributors.columns else 0
                        st.metric(
                    ...
                    with col2:
                        if branch_columns[1] in active_contributors.columns:
                    ...
                    with col3:
                        total_branches = total_active + (total_stale if isinstance(total_stale, (int, float)) else 0)
                        health_ratio = (total_active / total_branches * 100).round(1) if total_branches > 0 else 0
                        st.metric(
                            label="Branch Health", 
